...
class MenPageViewController: UIViewController {
    ...
    @IBOutlet weak var collectionView: UICollectionView! {
        didSet {
            ...
            layout.sectionInset = UIEdgeInsets(top: 28, left: 20, bottom: 20, right: 20)
    ...
    func loadData() {
        if let nextPage = nextPage {
            self.marketManager.getProductListData(page: nextPage, category: "men")
        }
        nextPage = 1
    }
...
extension MenPageViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if products.isEmpty {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "MenPageCell", for: indexPath) as! MenPageCell
        ...
        if let imageUrl = URL(string: product.mainImage) {
            cell.menImageView.kf.setImage(with: imageUrl, placeholder: UIImage(named: "Image_Placeholder"))
        }
...
extension MenPageViewController: UICollectionViewDelegateFlowLayout {
...
extension MenPageViewController: MarketManagerDelegate {
...
extension MenPageViewController: UICollectionViewDelegate {
    
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let flattenedProducts = products.flatMap { $0 }
        let selectedProduct = flattenedProducts[indexPath.item]
        performSegue(withIdentifier: "MenDetailSegue", sender: selectedProduct)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "MenDetailSegue" {
