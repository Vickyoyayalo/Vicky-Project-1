...
class WomenPageViewController: UIViewController {
    ...
    func loadData() {
        if let nextPage = nextPage {
            self.marketManager.getProductListData(page: nextPage, category: "women")
        }
        self.nextPage = 1
    }
...
extension WomenPageViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if products.isEmpty {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "WomanPageCell", for: indexPath) as! WomenPageCell
        ...
        if let imageUrl = URL(string: product.mainImage) {
            cell.womanImage.kf.setImage(with: imageUrl, placeholder: UIImage(named: "Image_Placeholder"))
        }
...
extension WomenPageViewController: UICollectionViewDelegateFlowLayout {
...
extension WomenPageViewController: MarketManagerDelegate {
...
extension WomenPageViewController: UICollectionViewDelegate {

    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let flattenedProducts = products.flatMap { $0 }
        let selectedProduct = flattenedProducts[indexPath.item]
        performSegue(withIdentifier: "WomanDetailSegue", sender: selectedProduct)
    }

    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "WomanDetailSegue" {
